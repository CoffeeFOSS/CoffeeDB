...
public class RoasterService(IUnitOfWork unitOfWork) : IRoasterService
{
  ...
  public async Task<ServiceResult<RoasterRevisionSnapshotDto>> CreateInitialRoasterRevisionAsync(CreateRoasterRevisionDto createRoasterRevisionDto, ClaimsPrincipal userClaims)
  {
    ...
    var revisionMetadata = unitOfWork.RevisionMetadataRepository.CreateRevisionMetadataAsync(createRoasterRevisionDto.Comment, userId, null);
    ...
    var roasterRevision = unitOfWork.RoasterRepository.CreateInitialRoasterRevisionAsync(revisionMetadata, createRoasterRevisionDto);
  ...
  public async Task<ServiceResult<RoasterRevisionSnapshotDto>> CreateRoasterRevisionAsync(int id, CreateRoasterRevisionDto createRoasterRevisionDto, ClaimsPrincipal userClaims)
  {
    ...
    var revisionMetadata = unitOfWork.RevisionMetadataRepository.CreateRevisionMetadataAsync(createRoasterRevisionDto.Comment, userId, currentRoasterId);

    if (!await unitOfWork.SaveAllAsync())
      return ServiceResult<RoasterRevisionSnapshotDto>.Failure(500, $"Unable to create Entity Revision Metadata for Roaster ID '{id}'.");

    var roasterRevision = unitOfWork.RoasterRepository.CreateRoasterRevisionAsync(id, revisionMetadata, createRoasterRevisionDto);
  ...
  public async Task<ServiceResult<RoasterRevisionSnapshotDto>> UpdateRoasterRevisionAsync(int revisionId, CreateRoasterRevisionDto createRoasterRevisionDto, ClaimsPrincipal userClaims)
  {
    ...
    var revisionMetadata = await unitOfWork.RevisionMetadataRepository.UpdateRevisionMetadataAsync(revisionId, createRoasterRevisionDto.Comment, userId);
  ...
  public async Task<ServiceResult<RoasterDto>> ApproveCreateRoasterRevisionAsync(int revisionId, ClaimsPrincipal userClaims)
  {
    ...
    var roasterRevision = await unitOfWork.RoasterRepository.GetRoasterRevisionEntityAsync(revisionId);
    if (roasterRevision == null)
    ...
    if (roasterRevision.RoasterId != null)
      return ServiceResult<RoasterDto>.Failure(400, $"Cannot create Roaster because Roaster Revision with ID '{revisionId}' is tied to an existing Roaster with ID '{roasterRevision.RoasterId}'.");

    var userId = UserClaimsUtils.GetUserId(userClaims);

    // Updates to the DB

    var roaster = unitOfWork.RoasterRepository.CreateRoaster(roasterRevision);

    var approvalResult = await unitOfWork.RevisionMetadataRepository.ApprovePendingRevisionMetadataAsync(roasterRevision.Id, userId, null);
    if (!approvalResult)
      return ServiceResult<RoasterDto>.Failure(400, $"Revision Metadata with ID {roasterRevision.Id} does not exist");

    if (!await unitOfWork.SaveAllAsync())
      return ServiceResult<RoasterDto>.Failure(500, $"Could not create Roaster using Roaster Revision ID {roasterRevision.Id}.");
  ...
  public async Task<ServiceResult<RoasterDto>> ApproveUpdateRoasterRevisionAsync(int roasterId, int revisionId, ClaimsPrincipal userClaims)
  {
    ...
    if (oldParentRevisionId != null)
    {
      await unitOfWork.RevisionMetadataRepository.AdoptPendingRevisionMetadatasAsync(oldParentRevisionId.Value, revisionId, userId);
    }
